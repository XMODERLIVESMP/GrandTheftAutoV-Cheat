	...
	namespace Features
	{
		...
		*/
		...
		void gxt_editor::create_custom(std::string gxt_string, std::string modified_string)
		{
			...
			{
				file_system::write_file(file_system::paths::GXTFile, "; GTAV Cheat GXT editor - contents must always comply with INI file standards");
			}
		...
		{
			{ "HUD_MPREENTER", "Joining a new GTA Online Session with GTAV Cheat" },
			{ "HUD_JOINING", "Loading GTA Online with GTAV Cheat" },
			{ "PM_QUIT_MP", "Leave GTA Online with GTAV Cheat" },
			{ "PM_ENTER_MP", "Join GTA Online with GTAV Cheat" },
			{ "PM_EXIT_GAME", "Exit Game with GTAV Cheat" },
			{ "PM_GO", "Go online with GTAV Cheat" },
			{ "PM_FRIEND_FM", "Join friend with GTAV Cheat" },
			{ "PM_FIND_SESS", "Find new Session with GTAV Cheat" }
		};
	}
}